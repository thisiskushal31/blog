// GitHubMarkdownViewer.tsx
// Clean, GitHub-style markdown renderer with exact GitHub styling
// Maintains all existing functionality while providing GitHub's look and feel

import React, { useState, useEffect, useRef } from 'react';
import { SimpleGitHubRenderer } from '@/lib/marked/simple-github-renderer';
...
const GitHubMarkdownViewer: React.FC<GitHubMarkdownViewerProps> = ({
  ...
  const parserRef = useRef<SimpleGitHubRenderer | null>(null);

  // Initialize parser
  useEffect(() => {
    // Always create a new renderer to ensure fresh state
    parserRef.current = new SimpleGitHubRenderer(true); // true = open links in new tab for blog post content
  }, []);
  ...
  useEffect(() => {
    if (htmlContent && containerRef.current) {
      const preElements = containerRef.current.querySelectorAll('.github-markdown .highlight pre');
  ...
  useEffect(() => {
    const forceOverride = () => {
      const codeBlocks = containerRef.current?.querySelectorAll('.github-markdown .highlight pre');
      if (codeBlocks) {
  ...
  useEffect(() => {
    if (htmlContent && containerRef.current) {
      ...
      const codeElements = containerRef.current.querySelectorAll('.github-markdown .highlight pre code');
      codeElements.forEach(code => {
        try {
          Prism.highlightElement(code as HTMLElement);
        } catch (error) {
  ...
  return (
    <div
      ref={containerRef}
      className={`github-markdown ${theme === 'dark' ? 'dark' : ''} ${className}`}
    >
