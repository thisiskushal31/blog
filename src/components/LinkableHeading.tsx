...
const LinkableHeading = ({ level, children, id, className = "" }: LinkableHeadingProps) => {
  // Only wrap the rendering logic in try/catch for error boundaries
  try {
  const Tag = `h${level}` as keyof JSX.IntrinsicElements;
  return (
      <Tag id={id} className={`group relative scroll-mt-8 flex items-center gap-2 ${className}`}>
        <span>{children}</span>
        {id && (
      <a
            href={`#${BLOG_BASE_PATH}#${id}`}
            ...
            onClick={(e) => {
              e.preventDefault();
              // Use the same dynamic approach as the main redirection logic
              const baseUrl = `${window.location.origin}${BLOG_BASE_PATH}`;
              const url = `${baseUrl}#${BLOG_BASE_PATH}#${id}`;
